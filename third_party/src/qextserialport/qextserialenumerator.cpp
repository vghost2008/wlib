...
#include <initguid.h>
	...
	QString QextSerialEnumerator::getRegKeyValue(HKEY key, LPCTSTR property)
	{
		...
		if (RegQueryValueEx(key, property, nullptr, nullptr, buff, & size) == ERROR_SUCCESS) {
	...
	QString QextSerialEnumerator::getDeviceProperty(HDEVINFO devInfo, PSP_DEVINFO_DATA devData, DWORD property)
	{
		...
		if (!SetupDiGetDeviceRegistryProperty(devInfo, devData, property, nullptr, buff, buffSize, nullptr))
	...
	void QextSerialEnumerator::setupAPIScan(QList<QextPortInfo> & infoList)
	{
		...
		devInfo = SetupDiGetClassDevs(guidDev, nullptr, nullptr, DIGCF_PRESENT | DIGCF_DEVICEINTERFACE);
		...
		SP_DEVICE_INTERFACE_DETAIL_DATA * detData = nullptr;
		DWORD detDataSize = 0;
		DWORD oldDetDataSize = 0;
		
		for (DWORD i = 0; ok; i++) {
			ok = SetupDiEnumDeviceInterfaces(devInfo, nullptr, guidDev, i, &ifcData);
			if (ok) {
				SP_DEVINFO_DATA devData = {sizeof(SP_DEVINFO_DATA)};
				//check for required detData size
				SetupDiGetDeviceInterfaceDetail(devInfo, & ifcData, nullptr, 0, & detDataSize, & devData);
